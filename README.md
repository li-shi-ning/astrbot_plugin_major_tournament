...
- **报名不限人数**：默认报名阶段不设人数上限，管理员开赛时再按实际报名人数自动确定规模（2/4/8/16/32/64 强）。支持自定义显示名、管理员代报名。
...
| `major 名单` | 所有人 | 查看报名名单 |
| `major 开赛 [名称] [规模]` | 管理员 | 开赛；名称可选、规模可选且放最后（不填规模按报名人数自动确定） |
| `major 对阵` | 所有人 | 文字版赛程 |
| `major 图` | 所有人 | 渲染 Major 对阵图 |
| `major 胜 <编号> <1\|2\|名字> [比分]` | 管理员 | 判定胜负，例：`major 胜 R16-3 1 2:1` |
| `major 命名 <名称>` | 管理员 | 自定义比赛名称（也可用 `major 开赛 4 我的杯` 顺带命名） |
| `major 重抽` | 管理员 | 重新随机抽签（仅开赛后、未产生真实结果前） |
...
| 👥 名单 / 📋 赛程 / 🖼 对阵图 | 查看类按钮 |
| 🚀 开赛 / ✏️ 自定义开赛 / 🗑 重置 | 仅管理员面板显示；「自定义开赛」点击后会把 `major 开赛 ` 填入输入框，可继续补名称与规模 |
